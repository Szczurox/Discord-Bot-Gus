    ...
    if member.permissions(ctx).expect("permissions").contains(Permissions::ADMINISTRATOR) {
        if args.is_empty() {
            ...
            if added_item == "money" || added_item == "mo" {
                ...
                lab.unwrap().remove_money(&amount).await;
                msg.channel_id.say(&ctx.http, &format!("✅ Successfully taken away from {} {} dollars", user.mention(), amount)).await?;
            }
            else if added_item == "meth" || added_item == "me" {
                let amount = amount_arg.unwrap();
                lab.unwrap().remove_meth(&amount).await;
                msg.channel_id.say(&ctx.http, &format!("✅ Successfully taken away from {} {} pounds of meth", user.mention(), amount)).await?;
            }
            else {
                wrong_argument(msg, ctx, String::from("AMOUNT")).await;
                return Ok(())
